...
def finalize_action():
    ...
    # Update Nav
    # We look for the buttons/links. In clean file they are likely still BUTTONS or LINKS if I converted them?
    # Wait, I am cleaning 'action.html' which is a raw reference. So they are likely BUTTONS.
    # So I need to convert them to A tags first, OR find them and replace them.
    ...
    for btn in soup.find_all(['button', 'a'], role='tab'):
        ...
        if text in nav_links:
            ...
            # Update classes for Active state 'Action plan'
            if text == 'Action plan':
    ...
    # Also check if there's any specific cleanup for action plan (like spinners?)
    # reusing the logic from finalize.py for removing spinners if any
